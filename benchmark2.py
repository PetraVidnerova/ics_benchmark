...
def work(model, criterion, optimizer, data_loader, epochs, device):
...
def main(data_root, workers, epochs, batch_size, lr, momentum, weight_decay):
    ...
    dist.init_process_group(
        backend="nccl", init_method="tcp://127.0.0.1:23456", timeout=timedelta(seconds=10),
        rank=0, world_size=1
    )
    ...
    work(model, criterion, optimizer, data_loader, epochs, device_id)
